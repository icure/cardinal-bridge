...
*/
	...
	suspend fun getContact(sessionId: String, entityId: String): Contact? =
		sdk(sessionId).contact.tryAndRecover.getContact(entityId)

	suspend fun getContacts(sessionId: String, entityIds: List<String>): List<Contact> =
		sdk(sessionId).contact.tryAndRecover.getContacts(entityIds)
...
*/
	...
	suspend fun filterContactsBy(sessionId: String, filter: AbstractFilter<Contact>): List<Contact> =
		getFromMatches(matchContactsBy(sessionId, filter)) { sdk(sessionId).contact.tryAndRecover.getContacts(it) }

	// Service-specific

	suspend fun getService(sessionId: String, serviceId: String): Service? =
		sdk(sessionId).contact.tryAndRecover.getService(serviceId)

	suspend fun getServices(sessionId: String, serviceIds: List<String>): List<Service> =
		sdk(sessionId).contact.tryAndRecover.getServices(serviceIds)
	...
	suspend fun filterServicesBy(sessionId: String, filter: AbstractFilter<Service>): List<Service> =
		getFromMatches(matchServicesBy(sessionId, filter)) { sdk(sessionId).contact.tryAndRecover.getServices(it) }

	// WithLinks

	private suspend fun withLinks(sessionId: String, contact: Contact): ContactWithLinks {
	...
	private suspend fun serviceWithLinks(sessionId: String, service: Service): ServiceWithLinks {
